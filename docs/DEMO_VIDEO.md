...
| 2 | 0:15–0:35 | Desktop `/` header | "One knowledge graph: 1,314 nodes, 100% of equipment linked across at least three document types." | Cursor traces the header stat + the live graph pane. |
...
| 9 | 2:45–3:00 | Deck final slide | "77% extraction recall, hybrid beats baseline where questions get hard, compliance one-for-one, sub-second answers, 82 tests — and it all runs offline. The plant already knows. Now it can tell you." | End card with the three novelties + repo/URL. |
